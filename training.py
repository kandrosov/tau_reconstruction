...
def training(mode, filename, parameters):
    ...
    model = MyGNN(mode=mode, map_features = _map_features, filename = filename, parameters = parameters) # creates the model
    ...
    if(mode=="dm"):
        model.compile(optimizer=tf.optimizers.Adam(learning_rate=0.001), loss=CustomMSE(mode= "dm"), 
                        metrics=[my_acc, my_mse_ch, my_mse_neu])
    else:
        model.compile(optimizer=tf.optimizers.Adam(learning_rate=0.001), loss=CustomMSE(mode= "p4"), 
